...
----------------------------
Keep the dataset tree intact so labels can be inferred from folders:

<data_root>/
  Fall/
    Bed/Mask Video/*.csv
    Chair/Mask Video/*.csv
    Stand/Mask Video/*.csv
  No Fall/
    Bed/Mask Video/*.csv
    Chair/Mask Video/*.csv
    Stand/Mask Video/*.csv
...
----------------------
The original starter script assumed a wide CSV format (one row per frame with many x/y columns).
Your actual FallVision samples are long-format (one row per keypoint), and some frames contain
multiple candidate skeletons. This script handles both issues by:
1) reconstructing candidate skeletons per frame,
2) selecting the best candidate using confidence,
3) pivoting to one skeleton per frame,
4) extracting lightweight temporal features,
5) training CPU-friendly baseline models.

Notes
-----
- Grouped splitting is done at the sequence/file level by default. That prevents leakage between
  windows from the same video.
- If later you find true subject IDs in metadata, you can replace sequence-level grouping with
  subject-level grouping for a stricter evaluation.
"""
...
def infer_group_id(path: Path) -> str:
    # Safe default: use sequence/file stem so windows from the same video stay together.
    return path.stem
...
def split_frame_into_candidates(frame_df: pd.DataFrame) -> List[pd.DataFrame]:
    """
    Reconstruct candidate skeletons within one frame.

    Observed FallVision sample behavior:
    - some frames have exactly 17 rows (single person),
    - some frames have 34/51/68/... rows (multiple detections),
    - rows appear in detection blocks of keypoints.

    We start a new candidate whenever a keypoint repeats.
    """
    ...
    for cand in candidates:
        # If the same keypoint somehow appears twice inside a candidate, keep the higher confidence row.
        cand = (
...
def long_pose_csv_to_wide_sequence(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    df = normalize_columns(df)
    validate_long_format(df)
    df = df[["Frame", "Keypoint", "X", "Y", "Confidence"]].copy()
    ...
    # Light interpolation keeps the pipeline stable when a few keypoints disappear briefly.
...
def build_frame_feature_table(wide_df: pd.DataFrame, conf_threshold: float = 0.20) -> pd.DataFrame:
    ...
    # Remove very low-confidence points from geometric calculations.
...
def build_dataset(data_root: Path, window: int, stride: int, conf_threshold: float) -> SequenceResult:
    ...
    for csv_path in find_csv_files(data_root):
        ...
        group_id = infer_group_id(csv_path)

        try:
            raw_df = pd.read_csv(csv_path)
            wide_df, diag_df = long_pose_csv_to_wide_sequence(raw_df)
